...
app.post("/register", async (req, res) => {
	...
	const users: any[] = await sequelize.query(
		`SELECT ID, PasswordHash, AdministratorAccess FROM Users WHERE Email = '${email}'`,
	...
	if (users.length === 0) {
		...
		let res = await sequelize.query(
			`INSERT INTO Users (Email, PasswordHash) OUTPUT inserted.ID VALUES ('${email}', '${hash}')`
		);

		await addCasesToUser((res[0] as any[])[0].ID, "Weapon Case", 1);
		await addCasesToUser((res[0] as any[])[0].ID, "Bravo Case", 1);
		await addCasesToUser((res[0] as any[])[0].ID, "Hydra Case", 2);
	} else {
...
app.post("/login", async (req, res) => {
	...
	const users: any[] = await sequelize.query(
		`SELECT AdministratorAccess, ID, PasswordHash FROM Users WHERE Email = '${email}'`,
		{ type: QueryTypes.SELECT }
	);
...
app.delete("/case", authenticateToken, async (req, res) => {
	...
	let intventoryDetails: any[] = await sequelize.query(
		`SELECT InventoryDetails.ID, InventoryDetails.Quantity FROM Users
				...
				INNER JOIN Cases
				ON InventoryDetails.CaseID = Cases.ID
				WHERE Cases.CaseName = '${caseName}' AND Users.Email = '${email}'`,
		{ type: QueryTypes.SELECT }
	);
	...
	for (let i = 0; i < intventoryDetails.length; i++) {
		if (intventoryDetails[i].Quantity === 1 && !removed) {
			...
			await sequelize.query(
				...
				} WHERE InventoryDetails.ID = ${intventoryDetails[i].ID}`
			);
			removed = true;
